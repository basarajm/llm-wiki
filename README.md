...
![한국 기업 리서치 위키 — 지식 그래프](docs/graph.png)

*212개 노드(기업·그룹·주주·산업·제품·공급망)의 연결 구조. 색상 = 페이지 타입, 노드 크기 = 연결도(degree).
허브로 보이는 국민연금공단·삼성전자·현대자동차·KB금융지주 등이 다수 엔티티를 잇습니다.
정적 이미지는 `engine\scripts\build-graph-image.py`로 재생성하며, 마우스로 탐색 가능한 인터랙티브 버전은 `wiki\viz.html`입니다.*
